...
import { FaChartLine, FaCheckCircle, FaTimesCircle, FaTrophy, FaArrowUp, FaArrowDown, FaWallet, FaLock, FaCoins } from 'react-icons/fa';
...
const TradeStats = () => {
    ...
    const statCards = [
        { label: 'Current Balance', value: `KES ${stats?.currentBalance?.toFixed(2) || '0.00'}`, icon: <FaWallet />, color: 'from-yellow-400 to-yellow-600' },
        ...
        { label: 'Total Loss', value: `KES ${stats?.totalLoss?.toFixed(2) || '0.00'}`, icon: <FaArrowDown />, color: 'from-red-400 to-red-600' },
        { label: 'Net Profit', value: `KES ${stats?.netProfit?.toFixed(2) || '0.00'}`, icon: <FaCoins />, color: 'from-purple-400 to-purple-600' },
    ];
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center p-4 bg-green-50 rounded-lg">
                                ...
                                <span className="text-2xl font-bold text-green-600">KES {stats?.totalProfit?.toFixed(2) || '0.00'}</span>
                            </div>
                            <div className="flex justify-between items-center p-4 bg-red-50 rounded-lg">
                                <span className="text-red-800 font-medium">Total Loss</span>
                                <span className="text-2xl font-bold text-red-600">KES {stats?.totalLoss?.toFixed(2) || '0.00'}</span>
                            </div>
                            <div className="flex justify-between items-center p-4 bg-blue-50 rounded-lg">
                                ...
                                <span className={`text-2xl font-bold ${(stats?.netProfit || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                    KES {stats?.netProfit?.toFixed(2) || '0.00'}
                                </span>
                            </div>
                            ...
                            <div className="flex justify-between items-center p-4 bg-purple-50 rounded-lg">
                                <span className="text-purple-800 font-medium">Total Traded</span>
                                <span className="text-2xl font-bold text-purple-600">KES {stats?.totalAmount?.toFixed(2) || '0.00'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-6">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                ...
                                <span className="font-bold text-green-600">KES {stats?.avgProfit?.toFixed(2) || '0.00'}</span>
                            </div>
                            <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                <span className="text-gray-600">Avg Loss</span>
                                <span className="font-bold text-red-600">KES {stats?.avgLoss?.toFixed(2) || '0.00'}</span>
                            </div>
                        </div>
                    </div>
                </div>
