...
import { FaWallet, FaChartLine, FaTrophy, FaUsers } from 'react-icons/fa';
...
const Dashboard = () => {
    ...
    const statCards = [
        { label: 'Balance', value: `$${user?.balance?.toFixed(2) || '0.00'}`, icon: <FaWallet className="text-2xl" />, color: 'from-yellow-400 to-yellow-600' },
                                    ...
                                    <tbody>
                                        {recentTrades.map((trade) => (
                                            <tr key={trade._id} className="border-t border-gray-100">
                                                ...
                                                <td className="py-2">
                                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${trade.status === 'open' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800'
                                                        }`}>
                                                        {trade.status}
                                                    </span>
                                                </td>
